...
# --- Configuration ---
K = 3
q_file = "combined_ixodes_all_variants_snps_passing_only.maf01.miss05.mac2.bi.LD_pruned_admixture.3.Q"
...
output_file = "admixture_K3_by_state_horizontal.jpg"
...
# Keep original column order to match K=2 pattern
# K3: col0=Southern(Red), col1=Northern(Blue), col2=New(Gold)
# This matches K=2 where after [1,0] swap we get Red(Southern) left, Blue(Northern) right
Q.columns = [0, 1, 2]
print("Column order: Southern(Red), Northern(Blue), New(Gold)")

# --- 5. Plot HORIZONTAL ---
# Red, Blue, Gold
unmc_palette = ['#AD122A', '#129DBF', '#F2A900']
...
# Place state labels using figure coordinates
