...
if K == 2:
    # K=2: col0 = Northern, col1 = Southern
    # Swap to: Southern (Green left), Northern (Purple right)
    col_order = [1, 0]
    print(f"K=2 column order: {col_order} (Southern/Green, Northern/Purple)")
    
elif K == 3:
    # K=3: col0 = Southern, col1 = Northern, col2 = New
    # Want: Southern (Green left), Northern (Purple middle), Orange (right)
    col_order = [0, 1, 2]  # Keep original
    print(f"K=3 column order: {col_order} (Southern/Green, Northern/Purple, Orange)")
    ...
    # Column dominant in TX = southern (green - left side)
    green_col = np.argmax(tx_means)
    # Column dominant in MA = northern (purple - right side)
    purple_col = np.argmax(ma_means)
...
# Green for Southern (left), Purple for Northern (middle), Orange for third (right)
if K == 2:
