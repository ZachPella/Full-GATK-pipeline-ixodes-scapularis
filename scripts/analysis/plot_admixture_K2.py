...
output_file = "admixture_K2_by_state.jpg"

# --- 1. Read Q-matrix ---
print(f"Reading Q-matrix from: {q_file}")
Q = pd.read_csv(q_file, sep='\s+', header=None)
...
fam_samples = fam[1].tolist()  # Second column has sample ID
...
# Create lookup: short name -> full label with state
meta_lookup = {}
for label in meta_labels:
    # Extract the sample name part (everything before _State)
...
# Match FAM samples to full labels
labels = []
for sample in fam_samples:
    # Handle special case for evi19055 -> 19055
...
# Swap columns so the ancestry components match fastSTRUCTURE orientation
Q = Q[[1, 0]]  # Swap column order
Q.columns = [0, 1]  # Reset column names

# --- 5. Plot ---
unmc_palette = ['#AD122A', '#129DBF']  # Red first (bottom), Blue second (top)

fig, ax = plt.subplots(figsize=(45, 6))
bottom = np.zeros(len(Q))

for i in range(K):
    ax.bar(range(len(Q)), Q.iloc[:, i], bottom=bottom, color=unmc_palette[i], width=1.0)
    bottom += Q.iloc[:, i]
...
for i in range(1, len(groups_sorted)):
    if groups_sorted[i] != current_group:
        ...
        display_label = current_group.replace('_', ' ')
...
for a in group_annotations:
    ax.text(a['center'], 1.08, a['label'], ha='center', va='bottom',
...
ax.set_xlabel("Samples (Sorted by Geography)", fontsize=10)
ax.set_title(f"ADMIXTURE K={K} - Grouped by State", fontsize=12, y=1.18)
